...
import br.com.sf.lojinha.ui.about.AboutFragment
...
internal constructor(private val mainActivity: MainActivity) : MainContract.Navigator {
    ...
    override fun goToProductDetail(product: Product) {
        ...
        mainActivity
            .customAppBar
            .setState(MainContract.Navigator.State.TWO_COLUMNS_WITH_DETAILS)
        mainActivity
            .containersLayout
            .state = MainContract.Navigator.State.TWO_COLUMNS_WITH_DETAILS

        val productsFragment = ProductDetailFragment.newInstance(product)
        val categoryFragment = ProductsFragment.newInstance(product.category)

        mainActivity
            .supportFragmentManager
            .beginTransaction()
            .replace(R.id.activity_main__frame_master, categoryFragment, TAG_MASTER)
            .commitNow()

        mainActivity
            .supportFragmentManager
            .beginTransaction()
            .replace(R.id.activity_main__frame_details, productsFragment, TAG_DETAILS)
            .commitNow()
    }
    ...
    override fun goToCategory(categoryId: Category) {
        ...
        val fragment = ProductsFragment.newInstance(categoryId)
